...
class TestPipelineConfig:
    def test_from_ticker(self, tmp_path: Path) -> None:
        config = PipelineConfig.from_ticker("CHC", tmp_path, years=5)
        assert config.tickers[0].ticker == "CHC"
    ...
    def test_to_dict(self, tmp_path: Path) -> None:
        config = PipelineConfig.from_ticker("CHC", tmp_path)
        d = config.to_dict()
        assert d["tickers"][0]["ticker"] == "CHC"
        assert d["years"] == 10
        assert "run_id" in d
...
class TestManifestBuilder:
    def test_build_manifest(self, tmp_path: Path) -> None:
        builder = ManifestBuilder("run_001", "CHC", tmp_path)
        mf = builder.build_manifest(
            ...
            relative_path="originals/CHC/metrics/ohlcv.parquet",
            byte_size=1024,
            sha256="a" * 64,
        ...
        assert "CHC" not in mf["company_id"]
...
class TestCoverageReporter:
    def test_build_report(self, tmp_path: Path) -> None:
        reporter = CoverageReporter("CHC", tmp_path)
...
class TestIdentityAtlasBuilder:
    def test_build_from_metadata(self, tmp_path: Path) -> None:
        builder = IdentityAtlasBuilder("CHC", tmp_path)
        yf_meta = {
            "short_name": "NVIDIA",
...
class TestResidualScanner:
    def test_scan_all_no_leaks(self, tmp_path: Path) -> None:
        ...
        reg = EntityRegistry.create("CHC", "test-reg")
        reg.add_entity("nvda_company", EntityType.COMPANY, "NVIDIA Corp")
        scanner = ResidualScanner("CHC", reg, tmp_path)
...
class TestNVIDIAReviewAdapter:
    def test_not_configured(self) -> None:
        ...
        try:
            ...
            result = adapter.review_batch(Path("/tmp"), "CHC")
