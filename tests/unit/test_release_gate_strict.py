...
class TestStrictReleaseGateBlocks:
    ...
    def test_fails_on_private_identity_map_in_public(self, tmp_path: Path) -> None:
        ...
        assert not result["passed"], f"Should block identity map in public: {result['fail_reasons']}"
...
class TestReleaseGateOutput:
    ...
    def test_gate_output_has_expected_keys(self, tmp_path: Path) -> None:
        ...
        result = evaluate_strict_release_gate(tmp_path)
        required = [
            "passed", "mode", "checked_at", "scanned_files", "scanned_bytes",
            "direct_identifier_hits", "metadata_hits", "forbidden_paths",
            "missing_required_files", "manifest_status", "fail_reasons", "gate_hash",
        ]
        for key in required:
